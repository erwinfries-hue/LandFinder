...
**newhome blockiert den Abruf in Produktion (2026-08-07, real beobachtet):** die ersten 4 echten newhome-Links landeten mit `BLOCKED`/`NOT_AVAILABLE` in `listings` — genau das im Portal-Zugriffs-Review (`docs/PORTAL_ACCESS_REVIEW.md`) erwartete Szenario. Migration `0004_listings_fetch_diagnostics.sql` ergänzt `last_fetch_http_status`/`last_fetch_at`, damit künftige Blockaden nachvollziehbar sind (z.B. 403 vs. 429 vs. Timeout), sichtbar auf der `/quellen/[id]`-Detailseite. Bewusst **nicht** umgesetzt: den User-Agent so zu ändern, dass er sich als normaler Browser tarnt, um die Blockade zu umgehen — das wäre genau der Tier-3-Bereich ("HTML-Scraping der Inserate direkt"), den der Portal-Review als hohes ToS-Risiko einstuft und der eine explizite Freigabe von dir braucht, keine stillschweigende Code-Änderung. Stufe 1 (Suchabo-Mail mit Original-Link) funktioniert für newhome weiterhin einwandfrei, unabhängig von diesem Fetch-Fehler.
