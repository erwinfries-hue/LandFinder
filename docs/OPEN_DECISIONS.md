...
Die Backfill-Route (`/api/admin/backfill-addresses`) berücksichtigt jetzt zusätzlich zu Adresse/Kanton auch Fläche und Zone: Kandidaten sind alle Zeilen, bei denen mindestens eines der drei Felder noch NULL ist; pro Zeile wird nur befüllt, was dort tatsächlich fehlt und was die erneute Extraktion liefert — nie ein bereits gesetztes Feld überschrieben.

**Bestätigt und erweitert (2026-08-16):** die "keine Fläche gefunden"-Hypothese oben war korrekt — der Auftraggeber teilte ein zweites Beispiel (Homegate-Link direkt), bei dem die Portal-Seite Wohnzone W1 **und** ein bestehendes Gebäude (Fachwerkhaus, Baujahr 1837 — eine starke Einschränkung bei einem als "Bauland" gelisteten Grundstück) zeigt, die nirgends in unseren Daten auftauchten. Ein direkter Abruf der echten Seite aus dieser Sandbox ist nicht möglich (`EGRESS_BLOCKED` bei `www.homegate.ch`, bestätigt) — Vercel (wo die App tatsächlich läuft) hat aber echten Internetzugang.

Zwei Ergänzungen: (1) `listingExtraction.ts` erkennt jetzt zusätzlich Baujahr (`matchBuildYear()`, Muster "Baujahr <Jahr>"/"aus dem Jahr <Jahr>", plausibler Bereich 1200–laufendes Jahr+1) und ein bestehendes Gebäude (`matchExistingBuilding()`, enge Keyword-Liste: Altbau/Bestandsbau/bestehendes Haus/vorhandenes Gebäude/Fachwerkhaus) — bleibt `undefined` statt `false`, wenn kein Hinweis im Text steht, da Abwesenheit eines Hinweises kein Beweis für die Abwesenheit eines Gebäudes ist. Sichtbar als neue Spalte "Geb." in der Quellen-Tabelle (Chip mit Baujahr, Hover-Text). (2) Neue Route `GET /api/admin/refetch-listing-pages` — anders als die Backfill-Route (durchsucht nur den bereits gespeicherten Mailinhalt erneut) ruft diese die echte Inserat-Seite **erneut ab**, läuft auf Vercel mit echtem Portalzugang. Kein neuartiges Scraping, nur ein wiederholter Versuch derselben, bereits freigegebenen Operation (Einzelseiten-Abruf der einen bekannten Inserat-URL) — z.B. für Zeilen, deren ursprünglicher Abruf blockiert war oder nur auf den knapperen Mailinhalt zurückfiel. Auf 8 Zeilen pro Aufruf begrenzt, gefahrlos wiederholt aufrufbar.

**Ergebnis (2026-08-16): Homegate blockiert auch von Vercel aus, definitiv bestätigt.** Erster Lauf (`{"checked":8,"refetched":8,"updated":0}`) — alle sechs echten Homegate-Inserat-Links kamen mit `HTTP 403` zurück (`status:"BLOCKED"`), auch mit browserähnlichem User-Agent und echtem Vercel-Internetzugang. Damit ist die frühere Vermutung ("vielleicht nur vorübergehend blockiert") widerlegt: Homegate setzt offenbar dieselbe Art robuster Bot-Erkennung wie newhome (siehe oben, vermutlich JS-Challenge/Cloudflare-artig) — ein einfacher Seitenabruf kann das grundsätzlich nicht umgehen, unabhängig vom Aufrufort. **Konsequenz:** Zone/Baujahr/bestehendes Gebäude lassen sich für Homegate-Inserate NICHT automatisiert nachträglich ergänzen, wenn sie nicht bereits im Mailtext standen — der einzige verbleibende Weg ist die manuelle Eingabe über "Objekt vertiefen" für Inserate, die der Auftraggeber näher prüfen will. Ein Versuch, die Blockade aktiv zu umgehen (z.B. Headless-Browser, IP-Rotation), würde die bereits getroffene Tier-1-Entscheidung (kein aggressives Scraping, siehe oben) verletzen und wird bewusst nicht verfolgt.

**Nebenfund dabei:** zwei der acht abgerufenen Zeilen waren gar keine echten Inserat-Seiten — ein Bild-Thumbnail (unproblematisch, bereits bekanntes Fallback-Verhalten) und ein newhome-Versanddienst-Link (`r.mailing.newhome.ch`), den `isPortalUrl()` fälschlich als echte newhome.ch-Seite durchliess (Subdomain-Match, obwohl kein Inserat) — dieselbe Fehlerklasse wie die Trefferlisten-Links. Behoben: `isMailingTrackingUrl()` filtert solche Versand-/Tracking-Subdomains jetzt komplett aus `extractPortalListingLinks()`. Die eine bereits bestehende Alt-Zeile mit diesem Link bleibt vorerst leer stehen (harmlos, kein Datenverlust) — bei Bedarf wie die Trefferlisten-Zeilen manuell aufräumbar.
...
- ~~Postmark-Limit erreicht~~ **erledigt (2026-08-16):** der geteilte Postmark-Account (auch von Tekmesis, Ponderio, HOME4efFINDER genutzt) hatte das monatliche Limit des kostenlosen "Developer"-Plans (100 Mails, Senden+Empfangen kombiniert) erreicht — LandFinders Suchabo-Pipeline (Punkt A) wäre bis zum 6.9.26 blockiert gewesen, da keine neuen Inbound-Mails mehr angenommen werden. Auf Empfehlung auf den "Basic"-Plan upgegradet: 10'000 Mails/Monat, darüber hinaus $1.80 pro 1'000 zusätzliche Mails. Betrifft nur den Inbound-Webhook (Suchabo-Mails) — ausgehende Alert-Mails laufen über Resend, nicht Postmark.
...
## D. Nutzerkreis — Login nur per E-Mail-Adresse
Private Web-App mit Login, aber einem globalen Suchprofil. Einziger Nutzer: der Auftraggeber persönlich (Rückmeldung 2026-08-17, siehe unten — löst die ursprüngliche Annahme "2-5 bekannte Nutzer" ab).

**Umgesetzt (2026-08-06):** ein geteiltes Passwort (Umgebungsvariable `APP_PASSWORD` in Vercel) schützt jetzt die ganze App inkl. `/api/state/*` — das bisher **ganz ohne Zugriffsschutz** erreichbar war (Suchprofil/Annahmen-Register liess sich ohne Login lesen und überschreiben). `apps/web/src/middleware.ts` prüft ein signiertes Session-Cookie (`lf_session`, HMAC-SHA256 über `apps/web/src/lib/authSession.ts`, Web-Crypto-API, 30 Tage gültig) vor jeder Seite/API-Route; `/login` und der Postmark-Webhook (`/api/inbound/*`, eigene Basic-Auth) bleiben ausgenommen. `/api/state/[id]` prüft das Cookie zusätzlich selbst (defense in depth). **Fail closed:** ist `APP_PASSWORD` nicht gesetzt, bleibt der Zugriff gesperrt statt offen.

**Nachgezogen (2026-08-17), auf Rückmeldung "nur für mich, E-Mail-Adresse genügt als Schutz":**
Login prüft jetzt nur noch, ob die eingegebene E-Mail-Adresse `erwin.fries@gmx.ch` entspricht (Konstante `ALLOWED_EMAIL` in `apps/web/src/app/api/auth/login/route.ts`) — kein Passwort-Feld mehr im Formular. `APP_PASSWORD` (Umgebungsvariable, Name aus der Passwort-Variante beibehalten, keine neue Vercel-Variable nötig) bleibt bestehen, dient aber nur noch als HMAC-Signierschlüssel für das Session-Cookie — nichts, was der Nutzer eingibt oder je zu Gesicht bekommt. Bewusst kein volles Multi-User-Login (einzelne Konten, Passwort-Reset) — für einen einzigen Nutzer unverhältnismässiger Aufwand.

**Bewusst akzeptierte Schwächung des Schutzniveaus:** eine bekannte E-Mail-Adresse ist kein Geheimnis (im Gegensatz zum bisherigen Passwort) — wer sie kennt oder errät, kommt ebenfalls hinein. Explizit vom Auftraggeber so gewünscht und als für den persönlichen Gebrauch ausreichend eingestuft.
