...
/** Löscht einen Quellenverzeichnis-Eintrag — mirrort DeleteRegionButton.tsx. */
export function DeleteQuelleButton({ quelleId, label }: { quelleId: string; label: string }) {
  const router = useRouter();
  const [deleting, setDeleting] = useState(false);

  async function handleDelete() {
    if (!window.confirm(`Quelle "${label}" wirklich löschen? Kann nicht rückgängig gemacht werden.`)) return;
    setDeleting(true);
    try {
      const res = await fetch(`/api/quellen/${quelleId}`, { method: "DELETE" });
