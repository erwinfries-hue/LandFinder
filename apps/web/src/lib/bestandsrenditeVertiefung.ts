...
/**
 * Manuell erfasste Bestandsrendite-Fakten (`listings.bestandsrendite`, Migration
 * 0009) — das Pendant zu `ListingVertiefungFacts` (listingVertiefung.ts) für
 * BESTANDSWOHNUNG statt Bauland/Abbruchobjekt. Jedes optionale Feld fällt auf den
 * ehrlich als Platzhalter markierten Default aus `BESTANDSRENDITE_PARAMETERS`
 * zurück (siehe `resolveBestandsrenditeParams`) — nichts wird erfunden, jede
...
export interface BestandsrenditeListingInput {
  kaufpreisChf: number;
 ...
 * (Schnellcheck/Investment Case/15-Jahres-Modell) plus Value-Add — das Pendant zu
 * `objektAnalysis.ts::computeListingAnalysis` für Bauland. Jede fehlende optionale
 * Annahme wird transparent durch den Platzhalter-Default aus
 * `BESTANDSRENDITE_PARAMETERS` ersetzt und in `assumptionNotes` offen ausgewiesen.
 */
export function computeBestandsrenditeAnalysis(listing: BestandsrenditeListingInput, facts: BestandsrenditeFacts): BestandsrenditeAnalysisResult {
  ...
  const kaufpreisChf = listing.kaufpreisChf + facts.parkplatzKaufpreisChf;
  ...
  const schnellcheck = calculateSchnellcheck({
    wohnungskaufpreisChf: listing.kaufpreisChf,
