...
export function DueDiligencePanel({
  ...
  async function handleUpload(event: React.FormEvent<HTMLFormElement>) {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        ...
        const res = await fetch(`/api/listings/${listingId}/documents`, { method: "POST", body: formData });
  ...
  async function handleSynthesize() {
    ...
    try {
      const res = await fetch(`/api/listings/${listingId}/due-diligence`, { method: "POST" });
  ...
  async function handleApplyProposal(field: string, newValue: string | number) {
    setApplying(field);
    try {
      await fetch(`/api/listings/${listingId}/due-diligence/apply-proposal`, {
        method: "POST",
